...
// path: pages/reels.jsx - توحيد معرف الريل لضمان عمل الفتح والمشاركة والإعجاب سواء رجع _id أو id
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="مقاطع قصيرة بملء الشاشة أو شبكة عرض شبيهة بإنستقرام"
        />
      ...
      {viewMode === 'reels' ? (
        <div className="reels-root">
          ...
          {!loading && reels.length === 0 && (
            <div className="reels-status">لا توجد مقاطع حالياً.</div>
          )}
          {error && (
            <div className="reels-toast" role="alert">
              {error}
            </div>
          )}
      ...
      <style jsx global>{`
        ...
        .dbl-heart {
          ...
          color: #ff2d55;
          pointer-events: none;
          animation: pop-heart 0.85s ease forwards;
          filter: drop-shadow(0 6px 18px rgba(255, 0, 80, 0.4));
          z-index: 40;
        }
        ...
        @keyframes pop-heart {
          ...
          100% {
            transform: translate(-50%, -90%) scale(1) rotate(-10deg);
            opacity: 0;
          }
        }
        ...
        .reel-progress {
          ...
          left: 0;
          height: 3px;
          background: linear-gradient(90deg, #00e0ff, #00ffa4);
          z-index: 62;
        }
        ...
        .reel-meta {
          ...
          align-self: end;
          max-width: 520px;
          padding-bottom: clamp(24px, 8vh, 60px);
        }
        ...
        .left-rail {
          ...
          top: calc(50% + var(--rail-shift, 0vh));
        ...
        .left-rail .rail-btn {
          ...
          font-size: 22px;
          cursor: pointer;
          box-shadow: inset 0 -2px 0 rgba(255, 255, 255, 0.06), 0 8px 18px rgba(0, 0, 0, 0.35);
...
function GridFeed({ items }) {
  ...
  const openReel = (id) => {
    router.push(
      { pathname: '/reels', query: { mode: 'reels', open: id } },
      undefined,
      { shallow: true }
    );
  };
            ...
            <div className="grid-overlay">
              <div className="grid-counts">
                <span>
                  ...
                  {formatCount(it.counts?.likes || 0)}
                </span>
                <span>
                  <i className="bx bx-show" />
                  {formatCount(it.counts?.views || 0)}
                </span>
              </div>

              <button className="grid-cta" onClick={() => openReel(it.id)}>
                مشاهدة
              </button>
            </div>
          </article>
...
function ReelCard({ reel, index, globalMuted, registerVideo, onToggleMute, fetchFp }) {
  const reelId = getReelId(reel, String(index));
  ...
  useEffect(() => {
    try {
      ...
      if (map[reelId]) setLiked(true);
    } catch {}
  }, [reelId]);

  const saveLikeLocal = (id, value) => {
    try {
      const raw = localStorage.getItem('reels_likes');
      const map = raw ? JSON.parse(raw) : {};
  ...
  useEffect(() => {
    if (videoRef.current) {
      registerVideo(reelId, videoRef.current);
      videoRef.current.muted = globalMuted;
    }
  }, [registerVideo, reelId, globalMuted]);
  ...
  const onLike = async (forceInc = false) => {
    ...
    saveLikeLocal(reelId, willLike);

    try {
      await fetchFp(`${base}/api/reels/${reelId}/like?dir=${willLike ? 'inc' : 'dec'}`, {
        method: 'POST',
      });
    } catch {}
  ...
  const onShare = async () => {
    try {
      ...
      if (shared) {
        ...
        try {
          await fetchFp(`${base}/api/reels/${reelId}/share`, { method: 'POST' });
        } catch {}
      }
    } catch {}
  };
  ...
  const handleTap = (e) => {
    if (e.target.closest && e.target.closest('.left-rail, .reel-cta, .circle, .rail-btn')) {
      return;
    }
    ...
    const clientX =
      e.clientX ?? (e.touches && e.touches[0]?.clientX) ?? rect.left + rect.width / 2;
    const clientY =
      e.clientY ?? (e.touches && e.touches[0]?.clientY) ?? rect.top + rect.height / 2;
    ...
    >
      <div className="reel-wrapper">
        ...
        <video
          ...
          poster={mediaUrl(reel.poster || reel.thumbnail)}
          playsInline
          loop
          preload="metadata"
          controls={false}
          ...
          <div className="reel-meta">
            <div className="reel-meta-row">
              <img
                ...
                height="32"
              />
              <strong className="reel-brand-name">@يؤثرون</strong>
            </div>
            ...
            {reel.ctaLink && (
              <Link href={reel.ctaLink} legacyBehavior>
                <a className="reel-cta">{reel.ctaText || 'المزيد'} ↗</a>
              </Link>
            )}
          </div>
        </div>
        ...
        <div className="left-rail" aria-label="شريط الإجراءات">
          ...
          <div className="rail-row">
            <button
              className={`rail-btn ${liked ? 'is-liked' : ''}`}
...
function CommentsDrawer({ reelId, open, onClose, onCountChange, fetchFp }) {
  ...
  const getSentCount = () => {
    try {
      return parseInt(localStorage.getItem(commentsQuotaKey) || '0', 10) || 0;
    } catch {
      return 0;
    }
  };
  ...
  const incSentCount = () => {
    try {
      ...
      localStorage.setItem(commentsQuotaKey, String(n));
    } catch {}
  };
  ...
  return (
    <aside className={`comments-drawer ${open ? 'open' : ''}`}>
      <div className="comments-header">
        ...
        <button className="circle" onClick={onClose} aria-label="إغلاق">
          <i className="bx bx-x" />
        </button>
      </div>
